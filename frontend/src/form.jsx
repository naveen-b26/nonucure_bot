...
function Form() {
  ...
  const handleInputSubmit = async (name, value) => {
    ...
    if (name === "phone") {
      ...
      if (!phoneRegex.test(value)) {
        setMessages((prev) => [
          ...prev,
          { type: "bot", content: "Please enter a valid 10-digit phone number." },
          {
            type: "bot",
            content: "What's your Phone number?",
            input: true,
    ...
    if (name === "age") {
      ...
      if (isNaN(age) || age < 16 || age > 90) {
        setMessages((prev) => [
          ...
          { type: "bot", content: "What's your age?", input: true, name: "age", placeholder: "Enter your age" },
        ])
        return
      }
    }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 px-4 py-6 sm:px-0">
      <div className="bg-black w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl text-white font-bold text-center text-xl sm:text-2xl font-mono p-3 rounded-t-lg">
        Self Assessment
      </div>
                          ...
                          >
                            {message.options.map((option) => (
                              <button
                                ...
                                className={`flex ${option.image ? "flex-col" : "flex-row"} items-center p-2 sm:p-3 bg-white rounded-lg hover:bg-gray-50 transition-colors text-gray-800 border text-sm sm:text-base`}
                              >
                                {option.image && (
